...
function validateFiles(files) {
  ...
  if (files.length > MAX_FILE_COUNT) return `첨부파일은 최대 ${MAX_FILE_COUNT}개까지 등록할 수 있습니다.`;
  for (const file of files) {
    const ext = getFileExtension(file.name);
    if (!ALLOWED_EXTENSIONS.includes(ext)) return `허용되지 않는 파일 형식입니다: ${file.name}`;
    if (file.type && !ALLOWED_FILE_TYPES.includes(file.type)) return `허용되지 않는 파일 형식입니다: ${file.name}`;
    if (file.size > MAX_FILE_SIZE) return `파일 용량은 1개당 6MB 이하만 가능합니다: ${file.name}`;
  }
  return null;
}
...
function renderSelectedFileList() {
  ...
  box.innerHTML = `
    <div class="selected-files-box ${fileError ? "file-error" : ""}">
      ...
      <ul>${files.map(file => `<li>${escapeHtml(file.name)} <span>${formatFileSize(file.size)}</span></li>`).join("")}</ul>
...
async function submitIdea() {
  ...
  const files = getSelectedFiles();
  const fileError = validateFiles(files);
  if (fileError) { setMessage("submitMsg", fileError, "error"); return; }
  const submitButton = document.querySelector("#submit button.primary");
  if (submitButton) { submitButton.disabled = true; submitButton.textContent = "제출 중..."; }
  try {
    const attachmentFiles = await uploadRawAttachments(files);
    const payload = {
      submitter_name: value("submitter_name"), employee_no: value("employee_no"), department: value("department"),
      category: value("category") || "LEVEL1", title, content,
      expected_effect: value("expected_effect"), expected_appearance: value("expected_appearance"),
    ...
    if (submitButton) { submitButton.disabled = false; submitButton.textContent = "제출"; }
  }
}

async function loadLikeSummary() {
  const { data, error } = await db.from("likes_summary").select("published_id, like_count");
  if (error) { likeSummaryCache = new Map(); return; }
...
async function loadBoard() {
  ...
  publishedCache = (ideas || []).map(idea => ({ ...idea, like_count: likeSummaryCache.get(idea.id) || 0 }));
  renderBoard();
}
...
function renderBoard() {
  ...
  const filtered = publishedCache.filter(idea => {
    ...
    return (!f || idea.category === f) && (!q || title.includes(q) || content.includes(q));
  });
  document.getElementById("boardList").innerHTML = filtered.map(idea => {
    const content = String(idea.content || "");
    const attachmentCount = Array.isArray(idea.attachment_files) ? idea.attachment_files.length : 0;
    return `
      <div class="idea-card" onclick="openDetail(${idea.id})">
        ...
        <p>${escapeHtml(content).slice(0, 110)}${content.length > 110 ? "..." : ""}</p>
        <div class="bottom"><span>${new Date(idea.published_at).toLocaleDateString()}</span><strong>${attachmentCount > 0 ? `📎 ${attachmentCount} ` : ""}♥ ${idea.like_count}</strong></div>
      </div>`;
...
async function likeIdea(id) {
  ...
  if (!employee_no) { setMessage("likeMsg", "사번을 입력해 주세요.", "error"); return; }
  const { error } = await db.from("likes").insert({ published_id: id, employee_no });
  if (error) {
    if (error.message.includes("duplicate") || error.code === "23505") setMessage("likeMsg", "이미 이 아이디어에 공감하셨습니다.", "error");
    else setMessage("likeMsg", "공감 저장 오류: " + error.message, "error");
    return;
  }
...
async function loadRanking() {
  ...
  const { data: ideas, error } = await db
    ...
    .eq("is_visible", true);
  if (error) { document.getElementById("rankList").innerHTML = "순위 로딩 오류: " + escapeHtml(error.message); return; }
  const rows = (ideas || []).map(idea => ({ ...idea, like_count: likeSummaryCache.get(idea.id) || 0 }))
  ...
  document.getElementById("rankList").innerHTML = rows.map((idea, index) => `
    <div class="rank-row"><div class="rank">${index + 1}</div><div><strong>${escapeHtml(idea.title)}</strong><br><span class="muted">${escapeHtml(idea.anonymous_no)} · ${escapeHtml(idea.category)}</span></div><strong>♥ ${idea.like_count}</strong></div>`).join("") || "순위 데이터가 없습니다.";
}
...
function renderPublishedAttachments(idea) {
  ...
  return `<div class="attachment-box"><h3>첨부파일</h3><div class="attachment-list">${files.map((file, index) => {
    ...
    if (fileType.startsWith("image/")) {
      return `<div class="attachment-item"><a href="${safeUrl}" target="_blank" rel="noopener noreferrer"><img src="${safeUrl}" alt="${fileName}" class="attachment-preview"></a><a class="file-link" href="${safeUrl}" target="_blank" rel="noopener noreferrer">${fileLabel}</a></div>`;
    }
    return `<div class="attachment-item"><a class="file-link" href="${safeUrl}" target="_blank" rel="noopener noreferrer">📎 ${fileLabel}</a></div>`;
  }).join("")}</div></div>`;
}
...
document.addEventListener("DOMContentLoaded", () => {
  initSupabase();
  const fileInput = document.getElementById("attachment_files");
  if (fileInput) fileInput.addEventListener("change", renderSelectedFileList);
});
